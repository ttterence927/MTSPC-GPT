...
class SupervisedRunner(BaseRunner):
    ...
    def train_epoch(self, epoch_num=None):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            halves = torch.chunk(X.view(X.size(0), -1), 2, dim=1)
            #print(int(halves[1].shape[1]))
            predictions, reg_pred = self.model(X.to(self.device), padding_masks)
            #print(halves[1].shape)
            loss_mse = mseloss(halves[1].to(self.device), reg_pred.to(self.device))

            loss = self.class_ratio * self.loss_module(predictions, targets) + (1- self.class_ratio) * loss_mse  # (batch_size,) loss for each sample in the batch
            batch_loss = torch.sum(loss)
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            #print(targets.shape)
            loss = self.class_ratio * self.loss_module(predictions, targets) + (1- self.class_ratio) * loss_mse
            batch_loss = torch.sum(loss).cpu().item()
