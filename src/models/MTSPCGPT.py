...
class MTSPCGPT(nn.Module):
    
    def __init__(self, config, data) :
        super(MTSPCGPT, self).__init__()
        self.pred_len = 0
        print(data.feature_df.shape)
        ...
        self.reg_layer = nn.Linear(config['d_model'] * self.patch_num, config['reg_size'])
    ...
    def forward(self, x_enc, x_mark_enc):
        ...
        #x_enc /= stdev
        #halves = torch.chunk(x_enc.view(x_enc.size(0), -1), 2, dim=1)
        ...
        reg_outputs   = self.reg_layer(outputs)
